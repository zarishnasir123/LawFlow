    ...
    <div className="space-y-4">
      ...
      <header className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div>
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-600">
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {pageCount} page{pageCount === 1 ? "" : "s"} to sign
                </span>
                {request.createdAt ? (
                  <span className="inline-flex items-center gap-1.5">
          ...
          {isSigned ? (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200">
              <CheckCircle2 className="h-3 w-3" />
              Signed
            </div>
          ) : (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-200">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_370px]">
        ...
        <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
          {/* Toolbar strip — names the surface and carries the page count
              so the paper area below stays chrome-free. */}
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 bg-gray-50/80 px-4 py-2.5">
            ...
            <span className="inline-flex items-center gap-2 text-[11px] text-gray-500">
              <span className="rounded-full bg-white px-2 py-0.5 font-semibold text-gray-600 ring-1 ring-gray-200">
                {pageCount} page{pageCount === 1 ? "" : "s"}
              </span>
              {isSigned
                ? "Read-only preview of your signed document"
                : "Drag your signature anywhere on a page"}
            </span>
          </div>

          <div
            className={`lawflow-signing-desk relative overflow-auto bg-slate-100 p-4 transition-all ${
        ...
        {/* ── Guided signing panel (or signed receipt) ─────────────── */}
        {isSigned ? (
