...
// Shared cache key so the panel always reflects the latest saved prefs.
const PREFERENCES_KEY = ["client", "notification-preferences"] as const;
...
// Only the categories that actually email the client. "Messages" is intentionally
// absent — chat is in-app only, there are no message emails.
const CATEGORIES: Array<{
  ...
  { key: "payment", label: "Payments", description: "Receipts and payment reminders", icon: CreditCard },
  { key: "document", label: "Documents & Signatures", description: "Signature requests and updates", icon: FileSignature },
];
...
// In-drawer settings view (rendered inside NotificationModal — no overlay of its
// own). Controls which EMAILS the client receives; the in-app bell always shows
// everything. On save it returns to the list via onSaved.
